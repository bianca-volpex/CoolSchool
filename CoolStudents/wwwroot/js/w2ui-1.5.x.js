...
*
//SVR
set method for list altered to select by id
    ...
    */
    ...
    w2field.prototype = {
        ...
        set: function (val) {
            if (['list', 'enum', 'file'].indexOf(this.type) != -1) {
                if (val && !val.id){ //SVR
                    var options = $.grep(this.options.items, function (e) { return e.id == val; });
                    if (options.length > 0)
                        $(this.el).data('selected', options[0]).change();                    
                } else
